...
■ 무엇이 가능하고 무엇이 불가능한가 (2026-08-26 API 실측)
  ...
  CME(해외)      **backfill 경로가 없다.** 이 계정에서 REST 조회 계열은
                  아무것도 주지 않는다 — o3121 마스터에 CME 없음 · o3106
                  현재가 빈 응답 · o3103 차트 0봉(2026-08-24·08-26 재확인).
                  유일한 소스가 실시간 WebSocket(OVC) tick 이므로
                  **수집기가 꺼져 있던 시간은 영구 결측**이다.
                  (--os-pages 시도는 남겨 두되 0행이면 조용히 넘어간다.)

  결론: **국내는 15시간 이내 구멍만 자동 복구된다. CME 는 복구 자체가 불가능**
