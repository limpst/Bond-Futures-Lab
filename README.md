...
**한 줄**: 독일 국채선물을 **실제 수집 대상으로 올렸고**, 그 과정에서 **CME 수집이 2.5일 멈춰 있었다는 사실**과 그 원인(월물 롤 미실행)을 찾았다.

#### 10.1 EUREX 3종 수집 개시 — 정본 DB 에 등록

8/27 실측에서 확인된 FGBL·FGBM·FGBS 를 `instrument` 에 올렸다. **정본 DB**(`bond-futures-lab/data/minbars.db`)에 넣는다 — 실험본은 스냅샷이므로 실시간 수집은 정본에만 둔다([§9.1](#91-저장소-분기--limpstbond-futures-lab)).

| instr_id | market | symbol | 상품 | 등록 근거 |
|---|---|---|---|---|
| FGBS | EUREX | `FGBSU26` | Euro-Schatz 2Y | 8/31 10:47 실측 105.325 |
| FGBM | EUREX | `FGBMU26` | Euro-Bobl 5Y | 8/31 10:47 실측 113.42 |
| FGBL | EUREX | `FGBLU26` | Euro-Bund 10Y | 8/31 10:47 실측 123.54 |

**월물 선택 근거**: 8/31 10:47 실측에서 `FGBLU26` 123.54(2 tick) vs `FGBLZ26` 122.70(1 tick) — **U26 이 아직 front**다. Z26 을 바로 등록하면 일주일간 얇은 계열을 쌓게 된다.

**코드 변경**
- `collect_cme_ws.py` — 담당 시장을 `CME` 에서 `WS_MARKETS = ("CME","EUREX")` 로 넓혔다. 둘 다 같은 TR(`OVC`) 한 줄로 들어오므로 거래소가 늘어도 코드가 갈라지지 않는다.
- `roll.py` — **EUREX 만기 규칙 추가**. ⚠️ Eurex 는 CME 와 규칙이 다르다:

| 거래소 | 만기 기준 | 2026-09월물 |
|---|---|---|
| CME | 최초인도통지일 = **인도월 직전월의 마지막 영업일** | 2026-08-31 |
| **Eurex** | 최종거래일 = **인도월 10일**(거래일 아니면 다음 거래일)**의 2거래일 전** | 2026-09-08 |

섞어 쓰면 롤이 이틀씩 어긋난다. 코드 규칙(`<root><월문자><연2자리>`)은 같아 파서는 재사용했다.

#### 10.2 🚨 발견 — CME 수집이 2.5일 멈춰 있었다

등록 작업 중 발견했다. **마지막 CME 봉 `2026-08-29 00:52`, 오늘(월) 07:00 재개장 이후 0봉.** 수집기 프로세스는 살아 있었고 watchdog 은 `stale` 만 15분마다 반복 기록하고 있었다.

**원인: 월물 롤 미실행.** `roll.py` 진단 결과 **CME 5종 전부 오늘(8/31)이 최초인도통지일**이었다 — 수집기가 이미 죽어가는 계약(`ZNU26` 등)을 구독하고 있었다. 프로브가 이를 독립적으로 확인해 줬다: `ZNZ26` 107.968750 **수신**, `ZNU26` **무음**.

**조치**
1. `roll.py --apply` — CME 5종을 U26 → Z26 으로 롤. 이음매(gap)는 `roll_event` 에 기록되어 back-adjust 로 처리된다(롤 자체의 손익은 0).
2. 수집기 재기동 → Z26 + EUREX 3종을 구독.

**⚠️ 미해결 — `roll.py` 가 스케줄에 없다.** 등록된 작업은 `CollectMinbars` · `CollectorGuard` · `MorningToken` · `OuSignal` · `CME bars` · `KTB day/night` 뿐이다. 롤은 **사람이 기억해야만 도는 수동 작업**이고, 이번에 아무도 기억하지 않아 2.5일이 날아갔다. Eurex 는 **9/8 에 같은 일이 반복될 예정**이다.

**⚠️ 미해결 — 관측 불가.** `CME bars` 작업이 `python -u tools\collect_cme_ws.py` 를 **직접** 실행해 stdout 이 어디에도 안 남는다. 로그를 남기는 `run_cme_ws.cmd` 러너가 있는데 작업은 그걸 안 쓴다. **2.5일 정지를 눈으로 볼 방법이 없었던 이유**가 이것이다.

#### 10.3 야간 프로브 예약 — 영국·캐나다 구멍 메우기

8/27 실측(13:42–13:55 KST)은 ICE·MX 개장 전이라 영국·캐나다를 판정하지 못했다. **평일 20:30 자동 실행**을 걸었다 — CBOT·Eurex·ICE·MX·OSE 야간이 **동시에 열리는 유일한 창**이다.

```
schtasks  BondFuturesLab\G7ProbeNight   평일 20:30
러너      tools/run_g7_probe_night.cmd
로그      tools/logs/g7_probe_YYYYMMDD.log
결과      reports/g7_bond_feed_YYYYMMDD.json   (날짜별 — 덮지 않는다)
```

**배치 구독 추가 (`--batch`, 기본 8)** — 8/27 의 "동시 구독 한도" 의심([§9.3](#93-g7-국채선물-universe-실측--신규-도구-toolsprobe_g7_bonds_wspy))에 대한 대응이다. 54종을 한 연결에 걸면 독일이 0 인데 6종으로는 수신됐으므로, 8종씩 끊어 순차로 돌고 결과를 병합한다. 만기도 전 국가 U26·Z26 둘 다 던진다(9월 초 롤 구간).

#### 10.4 오늘의 미검증 항목 (정직성)

| 항목 | 왜 미검증인가 |
|---|---|
| **8/29~8/31 CME 결측의 정확한 원인** | 롤 미실행이 오늘분은 설명하지만, **8/29 00:52 에 멈춘 것은 별개**다. 그 시점은 롤 전이었고 로그가 없어 확인 불가 |
| EUREX 봉의 장기 안정성 | 오늘 붙였을 뿐이다. 세션 경계·거래시간 처리가 Eurex 에서 맞는지 하루 이상 돌려봐야 안다 |
| Eurex 세션 정의 | `minbar` 의 세션 규율은 KTB(09:00–15:45 / 18:00–05:00) 기준이다. Eurex(08:10–05:00 KST)에 그대로 맞는지 **검토하지 않았다** |
| 20:30 프로브 첫 실행 | 오늘 저녁이 처음이다. 결과 미확인 |
| 실험본 repo 의 뒤처짐 | 정본이 5커밋 앞서 있다(`single_instance`·`flush_all` 수정 등). 실험본에 아직 안 들어왔다 |

#### 10.5 다음 단계

1. **`roll.py` 를 스케줄에 등록** — 이번 사고의 근본 원인. 매일 아침 `--apply` 로 돌리면 9/8 Eurex 롤도 자동 처리된다.
2. **`CME bars` 작업을 `run_cme_ws.cmd` 러너로 교체** — 로그가 남아야 다음 정지를 볼 수 있다.
3. Eurex 세션 정의를 `minbar` 규율에 반영.
4. 독일 커브(FGBS−FGBM−FGBL)에 ADF/OU/ECM-HAC 적용 — 표본이 쌓이는 대로.
5. 정본의 5커밋을 실험본으로 가져와 divergence 정리.
